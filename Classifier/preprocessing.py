...
def prepare_datasets(healthy, faulty, window_size, delay, feature_func):
    ...
    healthy_measures = pd.DataFrame([
        feature_func(healthy[window:window+window_size]) 
        for window in range(0, len(healthy) - window_size + 1, delay)
    ])
    ...
    healthy_measures = healthy_measures.sample(n=min_size, random_state=42)
    faulty_measures = faulty_measures.sample(n=min_size, random_state=42)
